...
export default function example() {
	...
	function draw() {
		...
		for (let i = 0; i < positionArray.length; i += 3) {
			positionArray[i] += Math.sin(time + randomArray[i] * 200) * 0.001;
			positionArray[i + 1] += Math.sin(time + randomArray[i + 1] * 200) * 0.001;
			positionArray[i + 2] += Math.sin(time + randomArray[i + 2] * 200) * 0.001;
		}
